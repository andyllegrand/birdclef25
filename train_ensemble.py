...
NUM_CLASSES = 207
BASE_OUTPUT_DIR = '/home/andy/Desktop/BirdClef/customSED/ensemble_runs_relabelled_ds'
...
labels_df = pd.read_csv("/home/andy/Desktop/BirdClef/customSED/data/precomputed_spectrograms/filtered_labels_base_models.csv")
...
skf = StratifiedKFold(n_splits=runs_per_model, shuffle=True, random_state=42)  # seed for reproducibility

for model_name in model_names:
    for run, (train_idx, val_idx) in enumerate(skf.split(counts_df, counts_df['label'])):
        try:
            train_counts_df = counts_df.iloc[train_idx].copy()
            val_counts_df = counts_df.iloc[val_idx].copy()

            # Filter segment-level labels
            train_labels_df = labels_df[labels_df['file_num'].isin(train_counts_df['file_num'])].copy()
            val_labels_df   = labels_df[labels_df['file_num'].isin(val_counts_df['file_num'])].copy()

            # Output directory
            out_dir = os.path.join(BASE_OUTPUT_DIR, f"{model_name}_fold{run}")
            os.makedirs(out_dir, exist_ok=True)

            # Train the model
            train_model(
                train_counts_df, train_labels_df,
                val_counts_df, val_labels_df,
                model_name, out_dir,
                lr, dropout, batch_size,
                wd=weight_decay, rs=random_sample
            )
        except Exception as e:
            print(f"Error training {model_name} (fold {run+1}) - {e}")